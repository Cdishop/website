...
\begin{equation}
y_t =  \alpha*constant_y + (1 + proportion_y)*y_{t-1} + coupling_xy*x_t{t-1} + e
\end{equation}
...
\begin{equation}
x_t =  \alpha*constant_x + (1 + proportion_x)*x_{t-1} + coupling_yx*y_t{t-1} + e
\end{equation}

\noindent where the terms are similar but now applied to values of $x$. The true values used in the DGP are:

\begin{align}
y_t &=  \alpha*0.5 + (1 + -0.32)*y_{t-1} + 0.4*x_t{t-1} + e \\
x_t &=  \alpha*0.5 + (1 + 0.22)*x_{t-1} - 0.4*y_t{t-1} + e
\end{align}
